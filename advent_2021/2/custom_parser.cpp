...
void N_custom_parsing::Parse_standard::parse_word_plus_num(ss &line, vector<N_input_parsing::Input_obj> &my_input)
{
    ...
    i ++;
    parsing_obj -> N_input_parsing::Input_obj::SetID(s_id);
    s_id.clear();
    while (i < line.length())
    {
        s_id += line[i ++];
    ...
    i_id = stoi(s_id, nullptr, 0);
    
    parsing_obj -> N_input_parsing::Input_obj::SetID(i_id);
